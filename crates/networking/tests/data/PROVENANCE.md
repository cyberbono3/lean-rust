...
| `handshake.status.ssz`               | `Status`          |

### `handshake.status.ssz` — generated, not copied

Unlike the other `wire-parity/` blobs, this one was not copied from a published corpus: it is
**generated by the reference implementation itself**, at the leanSpec revision pinned in
`README.md`, and is spec-derived in the stronger sense that this workspace's encoder had no part
in producing it.

Inputs are `finalized = (0x11 * 32, slot 7)` and `head = (0x22 * 32, slot 12)` — the two
checkpoints differ in both root and slot so a `finalized`/`head` swap cannot pass.

Regenerate with (`make spec-pq-devnet4` first, to get the checkout at the pinned revision):

```sh
cd leanSpec-pq-devnet-4
PYTHONPATH=src uv run --no-project --with pydantic --with numpy --with numba \
  --with cryptography --with pynacl --with aioquic python - <<'EOF'
import sys, types
# `Status` needs only `Checkpoint` and the SSZ types, but importing it pulls in
# `xmss/aggregation.py`, which imports the compiled `lean_multisig_py`. Building that
# extension needs a cargo newer than the one `uv` resolves, so stub the module: nothing
# on this path calls it.
stub = types.ModuleType("lean_multisig_py")
for name in ("aggregate_signatures", "setup_prover", "setup_verifier",
             "verify_aggregated_signatures"):
    setattr(stub, name, None)
sys.modules["lean_multisig_py"] = stub
...
s = Status(
    finalized=Checkpoint(root=Bytes32(bytes([0x11]) * 32), slot=Uint64(7)),
    head=Checkpoint(root=Bytes32(bytes([0x22]) * 32), slot=Uint64(12)),
)
data = s.encode_bytes()
assert len(data) == 80, len(data)
open("handshake.status.ssz", "wb").write(data)
EOF
```

Plain `uv run` inside the checkout does not work: it builds `lean-multisig-py`, which requires the
`edition2024` cargo feature, and the cargo `uv` resolves for that build was older than the
workspace toolchain. The `--no-project` invocation above sidesteps the project's own environment.
