...
use crate::core::error::{NodeError, ServiceFailure};
use crate::core::node::{NamedService, Node};
...
impl Node {
            ...
            match svc.start().await {
                ...
                Err(source) => {
                    ...
                    *self.state.lock() = None;
                    return Err(NodeError::StartFailed(ServiceFailure {
                        service: name,
                        source,
                    }));
                }
    ...
    ///
    /// # Errors
    /// [`NodeError::StopFailed`] if any service errors during shutdown.
    #[instrument(skip(self))]
    pub async fn stop(&self) -> Result<(), NodeError> {
        ...
        let cancel = derive_stop_token(self.config.shutdown_timeout);
        match stop_services_reverse(&started, &cancel).await {
            None => Ok(()),
            Some(failure) => Err(NodeError::StopFailed(failure)),
        }
    }
    ...
    ///
    /// # Errors
    /// [`NodeError::ServicesUnhealthy`] when one or more services report
    /// unhealthy.
    #[instrument(skip(self))]
    pub async fn status(&self) -> Result<(), NodeError> {
        let mut errors: Vec<ServiceFailure> = Vec::new();
        for (name, svc) in self.ordered_slots() {
            if let Err(source) = svc.status().await {
                errors.push(ServiceFailure {
                    service: name,
                    source,
                });
            }
...
/// Stops `services` in reverse order. Returns the first failure
/// encountered; subsequent failures are logged via [`tracing::warn`].
async fn stop_services_reverse(
    services: &[NamedService],
    cancel: &CancellationToken,
) -> Option<ServiceFailure> {
    let mut first_error: Option<ServiceFailure> = None;
        ...
        match stop_one(svc, cancel).await {
            ...
            Err(source) => {
                ...
                if first_error.is_none() {
                    first_error = Some(ServiceFailure {
                        service: name,
                        source,
                    });
                }
...
async fn stop_one(svc: &Arc<dyn Service>, cancel: &CancellationToken) -> anyhow::Result<()> {
    ...
    //
    // `biased` polls the stop future first: if the deadline has already
    // fired but the service can finish synchronously, we let it. Without
    // `biased`, tokio randomizes ready branches and we'd occasionally
    // return a deadline error even when the stop would have completed in
    // one poll.
    tokio::select! {
        biased;
