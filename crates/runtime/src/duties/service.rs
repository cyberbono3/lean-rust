...
//!
//! Lifecycle mirrors [`crate::sync::Loop`]: [`crate::core::Service`]
...
/// constructed `Service` behind `Arc<dyn crate::core::Service>`.
pub struct Service {
    config: Config,
...
/// which [`crate::core::Service::status`] flips `Ok → Err`. Tolerating
...
struct ServiceState {
    ...
    /// [`crate::core::Service::status`]. Reset on each `start`.
...
impl Drop for Service {
    ...
    /// through [`crate::core::Service::stop`]: cancel the shared token
...
/// here surfaces via [`crate::core::Service::status`] without the worker
...
mod tests {
    ...
    fn publisher() -> (TempDir, Arc<Publisher>) {
        ...
        let options = crate::p2p::HostOptions::try_new(
        ...
        .unwrap();
        let p2p = Arc::new(crate::p2p::DevnetHost::build(options).unwrap());
    ...
    async fn status_before_start_errors() {
        let (_dir, svc) = service();
        assert!(crate::core::Service::status(&svc).await.is_err());
    }
