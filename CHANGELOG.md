...
before it merges. Put one line at the TOP of the matching `####` heading in the current
`## vX.Y.Z (Unreleased)` section — entries run newest first, so the most recent PR is
always the first bullet under its heading:
...
- [INTEROP] Published attestations on the spec's `attestation_0` subnet topic instead of `vote`, a string no conformant peer subscribes to; this client's attestations previously reached nobody on a spec network, silently and without error, and nodes on either side of this change do not exchange attestations ([#188](https://github.com/cyberbono3/lean-rust/issues/188)).
- [INTEROP] Served block recovery on the spec's `/leanconsensus/req/blocks_by_root/1/ssz_snappy` protocol id rather than a client-specific `lean_blocks_by_root` variant, so a peer requesting a block by root now reaches this node instead of failing to negotiate a protocol ([#188](https://github.com/cyberbono3/lean-rust/issues/188)).
