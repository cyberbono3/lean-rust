...
# Both disposable and gitignored, and regenerated rather than kept: the spec
# checkout is derived from the revision recorded in README.md's interop table,
# and nothing durable may be written to either. SPEC_DIR sits at the repository
# root rather than under AUDIT_DIR so the tree on disk names the devnet
# generation it holds.
SPEC_DIR ?= leanSpec-pq-devnet-4
AUDIT_DIR ?= .audit
...
help:
	...
	@echo ""
	@echo "conformance targets:"
	@echo "  make spec-pq-devnet4       - check out the recorded leanSpec revision at $(SPEC_DIR)"
	@echo "  make spec-pq-devnet4-clean - remove $(SPEC_DIR) and $(AUDIT_DIR)"
	@echo ""
...
# Deliberately not a prerequisite of `verify`: this reaches the network on first
# run, and the canonical gate must stay offline and hermetic. Spec citations of
# the form `leanSpec@<rev>:<path>:<line>` are only checkable once this has run.
spec-pq-devnet4:
	scripts/fetch-leanspec.sh

# Safe to run at any time — both directories are regenerated by
# `spec-pq-devnet4`, and no output that is meant to survive is written to
# either. $(AUDIT_DIR) is removed as well as $(SPEC_DIR) because the checkout
# used to live under it: one command drops a tree left behind by the previous
# pin, and nobody has to know where it used to be.
spec-pq-devnet4-clean:
	rm -rf $(SPEC_DIR) $(AUDIT_DIR)
