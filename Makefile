...
# Disposable and gitignored. The spec checkout under it is derived from the
# revision recorded in README.md's interop table, so it is regenerated rather
# than kept, and nothing durable may be written here.
AUDIT_DIR ?= .audit
...
help:
	...
	@echo "  make audit-spec       - check out the recorded leanSpec revision at $(AUDIT_DIR)/leanSpec"
	@echo "  make audit-spec-clean - remove $(AUDIT_DIR)"
	@echo ""
...
# Safe to run at any time — everything under $(AUDIT_DIR) is regenerated by
# `audit-spec`, and no output that is meant to survive is written there.
audit-spec-clean:
	rm -rf $(AUDIT_DIR)
